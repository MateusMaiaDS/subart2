...
\noindent\textit{Note: This manuscript is an anonymized version prepared for peer review. Author-identifying information has been removed where required.}
...
\begin{aug}
\author{\fnms{Anonymous}~\snm{Author(s)}}
\address{Anonymous affiliation(s) withheld for peer review.}
\runauthor{Anonymous}
\end{aug}
...
A recently proposed extension of BART accounts for the correlation between costs and health outcomes in CEA data; this model is referred to as suBART \citep{esser2025seemingly}.
...
In this section we guide the reader through the use of suBART. We use a simulated dataset, which is designed as an example and not meant to represent any specific real-world research endeavour. In the supplementary material
\footnote{Supplementary code and data are provided in the online appendix (link withheld for anonymous review).}
, we provide the full dataset, together with all of the following code.
...
data_url <- "URL_WITHHELD_FOR_ANONYMOUS_REVIEW"
