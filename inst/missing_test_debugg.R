...
plot(x, y_hat[,1], col = ifelse(is.na(y_obs[,1]), alpha("red", 0.5), alpha("black", 0.5)),
     pch = 16, ylim = c(-15,15),main = "with imputation")
lines(x, fit$y_hat_mean[,1], col = "blue")
lines(x, y1_pred_band[,1], col = "blue", lty = "dashed")
lines(x, y1_pred_band[,2], , col = "blue", lty = "dashed")

plot(y[,1], fit$y_hat_test_mean[,1],
     col = ifelse(is.na(y_obs[,1]), alpha("red", 0.5), alpha("black", 0.5)),
     ylim = c(-15,15), main = "with imputation")
lines(y[,1], y[,1])
y1_pred_band <- matrix(NA, nrow = n, ncol = 2)
for ( i in 1:n ) {
...
#
# plot(x, y_hat[,1], col = ifelse(is.na(y_obs[,1]), alpha("red", 0.5), alpha("black", 0.5)),
#      pch = 16, ylim = c(-15,15), main = "without imputation")
